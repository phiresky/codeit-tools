...
System.register("bin/app.js", ["react", "react-dom", "react-select", "query-string"], function(exports_1, context_1) {
  ...
  function renderTableRow(data, id) {
    ...
    return React.createElement("tr", {key: id}, [id, data.date, data.map, data.mapSize, hrefs[0], data.resources[0], hrefs[1], data.resources[1], React.createElement("a", {href: `https://codeit.itdhosting.de:1337/replay.php?game=${id}`}, "Show Replay")].map((x) => React.createElement("td", null, x)));
  }
  ...
  function deserializeState(state) {
    ...
    return {
      ...
      sortColumn: obj.sort || "ID",
      sortReverse: typeof obj.reverse === "undefined" ? true : !!obj.reverse
    };
  }
    ...
    execute: function() {
      ...
      GUI = class GUI extends React.Component {
        constructor(props) {
          ...
          this.maps = [...new Set(Object.keys(data).map((k) => data[k]).map((game) => game.map))].map((map) => ({
            value: map,
            label: map
          }));
        ...
        render() {
          ...
          }))), React.createElement("hr", null), React.createElement("div", {className: "alert alert-info"}, "Found ", count, " games"), React.createElement("div", {className: "table-responsive"}, React.createElement("table", {
            className: "table",
            data: ids
          }, React.createElement("thead", null, React.createElement("tr", null, "ID,Date,Map,Map Size,Winner,Winner Resources,Looser,Looser Resources,Replay".split(",").map((name) => React.createElement("th", {key: name}, React.createElement("a", {
            href: "#",
            onClick: (e) => this.setSort(e, name)
          }, name, this.state.sortColumn === name ? this.state.sortReverse ? "▼" : "▲" : ""))))), React.createElement("tbody", null, ids.map((id) => renderTableRow(data[id], id))))), count > displayLimit ? React.createElement("div", {className: "alert alert-danger"}, count - displayLimit, " more not displayed.", " ", React.createElement("a", {
            href: "#",
            onClick: (e) => {
              e.preventDefault();
      ...
      }).then(() => fetch("../data/games.json")).then(jsonOrError).then((data) => render(React.createElement(GUI, {data: data}))).catch((e) => {
        console.error(e);
